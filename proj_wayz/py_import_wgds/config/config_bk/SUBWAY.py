...
# 读取的文件, 按需更改
result_file = 'E:/py_brand/data/out_data/SUBWAY.json'
...
def insert_info(row):
    ...
    name = "SUBWAY(" + CHECK.check_value(row.get('ShopNameCN', None)) + ')' #必填 字符串
    ...
    customized_id = CHECK.check_value(row.get('ShopID', None))  #必填 原始ID 字符串
    source_ids = [data_src+':'+edit_version+":"+ customized_id ] # 必填 来源+v0.1+原始ID 字符串数组

    try:
        lng = float(row.get('BaiduLon', None)) #必填 火星坐标 浮点
        lat = float(row.get('BaiduLat', None)) #必填 火星坐标 浮点
    except Exception as e:
