...
# 读取的文件, 按需更改
result_file = 'E:/py_brand/data/out_data/starbucks.json'
...
def insert_info(row):
    ...
    name = '星巴克('+CHECK.check_value(row.get('name', None))+')' #必填 字符串
    name_en = None  #可选 字符串
    code = None #可选 行政区划ID
    category_ids = [30600] #必填 整型数组 类别ID，参考WGDS规范
    category_names = ['咖啡馆'] #必填 字符串数组 与category_ids对应
    image_urls = None #可选 字符串数组
    brands = None #必填 整型数组 品牌ID
    opening_hours = CHECK.check_value(row.get('today', None).get('openTime','')+'-'+row.get('today', None).get('closeTime','')) #可选 字符串
    phone_numbers = [] # 可选 字符串数组
    #phone_numbers.append( CHECK.check_value( CHECK.del_chinese(row.get('TEL', '').replace(' ','')) ) )
    ...
    address2 = row.get('address', {}).get('streetAddressLine1','')
    address3 = row.get('address', {}).get('streetAddressLine2','')
    address4 = row.get('address', {}).get('streetAddressLine3','') # 必填 字符串
    if address_city and not address_city in address2:
    ...
    address = CHECK.check_value(address)
    price = None #可选 整型
    ...
    customized_id = CHECK.check_value(row.get('id', None))  #必填 原始ID 字符串
    source_ids = [data_src+':'+edit_version+":"+ customized_id ] # 必填 来源+v0.1+原始ID 字符串数组

    try:
        lng = float(row.get('coordinates', None).get('longitude',None)) #必填 火星坐标 浮点
        lat = float(row.get('coordinates', None).get('latitude',None)) #必填 火星坐标 浮点
    except Exception as e:
