...
# 读取的文件, 按需更改
result_file = 'E:/py_brand/data/out_data/pizzahut.json'

#  按需更改
def main(insert_sql):
    with open('config/cities.txt') as f:
        for line in f.readlines():                          #依次读取每行  
            line = line.strip()                             #去掉每行头尾空白  
            # ipt_data = list(BASE.read_csv(result_file))
            ipt_data = BASE.load_json_file(result_file)[line]
...
def insert_info(row):
    ...
    name = "必胜客(" + CHECK.check_value(row.get('restaurant_name', None)) + ')' #必填 字符串
    name_en = CHECK.check_value(row.get('ShopNameEN', None))  #可选 字符串
    code = None #可选 行政区划ID
    category_ids = [30200] #必填 整型数组 类别ID，参考WGDS规范
    category_names = ['快餐/小吃'] #必填 字符串数组 与category_ids对应
    image_urls = None #可选 字符串数组
    brands = [62200072] #必填 整型数组 品牌ID
    opening_hours = CHECK.check_value(row.get('StoreHours', None)) #可选 字符串
    phone_numbers = [] # 可选 字符串数组
    phone_numbers.append( CHECK.check_value( CHECK.del_chinese(row.get('phone', '').replace(' ','').replace("'","")) ) )
    phone_numbers = CHECK.check_value(phone_numbers)
    website = None # 可选 字符串
    chains = True # 可选 bool类型
    postcode = None #可选 整型
    business_area = None # 可选 字符串
    address = CHECK.check_value(row.get('address', None)) # 必填 字符串
    price = None #可选 整型
    ...
    customized_id = CHECK.check_value(row.get('sid', None))
    if customized_id:
        customized_id = customized_id.replace("'","")
     #必填 原始ID 字符串
    if customized_id:
        source_ids = [data_src+':'+edit_version+":"+ customized_id.replace("'","") ] # 必填 来源+v0.1+原始ID 字符串数组
    else:
        source_ids = [data_src+':'+edit_version+":null" ]
    try:
        lng = float(row.get('coordinate',None).split(',')[1])
        lat = float(row.get('coordinate',None).split(',')[0])
    except Exception as e:
